...
const handleReduceImageQuality = async (file: Blob, quality?: number): Promise<Blob | boolean> => {
...
export const handleFileInputUpload = async (
  ...
  isProcessedFiles?: boolean
    ...
    if (fileList) {
      ...
      if (isSomeImageTooLarge) return `Maximum image size allowed is ${fileSizeLimit.toUpperCase()}`
      return {
...
export const handleDragAndDropFileUpload = async(ev: any, quality?: number, fileLimit: number = 100, fileSizeLimit: string = "" ) => {
  ev.preventDefault();
  const filePromises = [...ev.dataTransfer.items].map((item) => item.getAsFile())
  return await handleFileInputUpload(filePromises, quality, fileLimit, fileSizeLimit, true)
}